...
| `TRAWLARR_PROBE_CONCURRENCY` | `scan.probeConcurrency` (seed)   | How many `ffprobe` processes a scan runs at once, 1–64, default `4`. Higher means a **faster first scan** — the only scan that probes every file — and **more IO contention with the transcodes running beside it**. A rescan probes nothing (unchanged files are skipped), so this affects the first walk of a library and little else. `1` is the strictly serial behaviour. Worth raising on a slow network mount, where each probe waits on round-trips rather than on this machine's cores; worth leaving alone on a local disk. |
...
| `:X`           | The newest release in that major series, from `1.0.0` on.    |
