...
/**
 * 2,000,000 bytes: BIGGER than the replacement below, the ordinary case on a
 * real library (152 of 157 replacements on the owner's Shows library shrank
 * the file). The readable phrase survives at the front so a failing content
 * assertion still says what the file was.
 */
...
describe('createReplaceOriginalRunner', () => {
  ...
  it('installs a replacement bigger than the original: the size limit belongs to the flow now', async () => {
    // A 2.0x replacement, which the built-in gate used to refuse. How much
    // bigger is acceptable is a Check Size Change node's call, placed before
    // this one; Replace Original File installs what it is handed.
    const space = workspace();
    ...
    expect(out.outputNumber).toBe(1);
    expect(statSync(space.originalPath).size).toBe(inflatedBody.length);
    // Still recoverable: the original went to trash, as for any replacement.
    const trashed = readdirSync(space.trashDir);
    expect(trashed).toHaveLength(1);
    expect(sha256(join(space.trashDir, trashed[0]!))).toBe(originalDigest);
  });
  ...
  it('refuses a replacement bigger AND truncated: being bigger does not excuse a broken encode', async () => {
    // Whatever size limit the flow allowed, a file that lost its running time
    // is refused — a flow permitting growth must not let a truncated encode in.
    const space = workspace();
