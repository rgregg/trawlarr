...
import { extractFacts, tdarrWorkerType, type FactSet, type PartialHashParts } from '@trawlarr/core';
import type {
  ConfigVars,
  PluginFileObject,
  PluginInputArgs,
 ...
 *
 * There is deliberately no database handle here, and no way to obtain one:
 * the ports are callbacks and plain functions, all of which Task 5 can
 * implement over an IPC channel to a parent process that DOES hold the
 * database. `documents` is async-tolerant for exactly that reason (see
 * `DocumentPort` in `@trawlarr/engine`).
 */
export interface RunPayloadPorts {
  ...
  nowMs: () => number;
  /** Cancellation, reaching ffmpeg itself through the Execute runner. */
  signal?: AbortSignal;
  /**
...
/**
 * What the Replace Original File node left on disk, as plain data.
 *
 * Present whenever a Replace node in this run REPORTED a path that can still
 * be `stat`'d — which is the run's whole knowledge of "what path now holds
 * the library file". Whether that constitutes a CHANGE to the tracked row is
 * deliberately NOT decided here: the comparison is against the row's stored
 * `content_key`/`inode_key`, which are database facts, so `applyJobReport`
 * makes it. `deviceId`/`inode`/`hash` are carried precisely so that it can:
 * they are exactly what `buildIdentityCandidate` consumes.
 *
 * `probe` is null when the replaced path could not be probed. That is not
 * fatal on its own — a Replace that REFUSED reports the untouched original,
 * and nothing about that run needs a fresh probe — so the error is carried
 * rather than thrown, and `applyJobReport` raises it only if the identity
 * comparison says the file really did change and the probe is therefore
 * load-bearing.
 */
export interface ReplacedFile {
  ...
  /** Why `probe` is null, for `applyJobReport` to raise if it needs the probe. */
 ...
 * touching a database.
 *
 * This is the half of the old `runJob` that does the work: it loads the
 * plugins, wires the engine's real execute/verify/replace runners in, runs
 * the flow, and inspects the filesystem afterwards to describe what Replace
 * Original File actually did. Every fact it needs arrives in `payload`;
 * everything it wants to tell the world goes out through `ports`. Nothing
 * in this module imports a repository, opens a connection, or takes a `Db`,
 * which is what lets Task 5 run it in a forked child process.
 *
 * It does not decide the file's next state either: that is a fold over the
 * ledger, which lives in the database. `applyJobReport` does it.
 */
export const runPayload = async (input: {
  payload: JobPayload;
